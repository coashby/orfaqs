...
if PlatformUtils.is_macos():
    ...
    class MetalComputeAccelerator(ComputeAccelerator):
        ...
        def init(
            self, device_name=None, kernel_source_file_path=None, **kwargs
        ):
        ...
        def max_number_threads_on_device(device_name: str = None) -> int:
            ...
            if compute_device is None:
                ...
                if device_name is not None:
                    message += (
                        f' The device name: "{device_name}" does not exist.'
                    )
        ...
        def _configure_compute_resources(self, function_name: str):
            ...
            self._current_pipeline_state = self._compute_device.newComputePipelineStateWithFunction_error_(
                kernel_function,
                None,
            )[0]
