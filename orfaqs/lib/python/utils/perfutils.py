...
class MetricSample:
    def __init__(self, timestamp: int, value: any, value_name: str = None):
        self._timestamp = timestamp
        self._value = value
        self._value_name = 'value'
        if isinstance(value_name, str) and (len(value_name.replace(' ')) > 0):
            self._value_name = value_name

    def __str__(self):
        return (
            f'timestamp: {self._timestamp}, {self._value_name}: {self._value}'
        )
    ...
    @property
    def map(self) -> dict:
        return {'timestamp': self._timestamp, self._value_name: self._value}
...
class ElapsedTimeSample:
    ...
    def start_time(self, value: int):
        if (self._start_time is not None) or (value is None):
            return
        self._start_time = value

    @end_time.setter
    def end_time(self, value: int):
        if (self._end_time is not None) or (value is None):
            return
...
class PerfProfiler:
    ...
    def csv_log_output_file_path(self) -> pathlib.Path:
        ...
        file_name = f'{self._name}-{self.log_file_index}'

        return output_directory.joinpath(file_name).with_suffix('.csv')

    @property
    def _csv_log_file_header(self):
        return 'function_name, start_timestamp (ns) ,end_timestamp (ns), elapsed_time (ns)'

    def _log_file_init(self):
        DirectoryUtils.mkdir_path(self.csv_log_output_file_path.parent)
        # Previous logs are overwritten.
        with open(
            self.csv_log_output_file_path, 'w', encoding='utf-8'
        ) as o_file:
            o_file.write(f'{self._csv_log_file_header}\n')

    def _update_log_file(
        self, function_name: str, elapsed_time_sample: ElapsedTimeSample
    ):
        with open(
            self.csv_log_output_file_path, 'a', encoding='utf-8'
        ) as o_file:
            o_file.write(
                (
                    f'{function_name}, '
    ...
    def function_elapsed_time_samples(
        self, key: str
    ) -> list[ElapsedTimeSample]:
        return self._elapsed_time_samples.get(key)

    def _validate_stop_perf_timer_call(
        self, function_name: str, samples_list: list
    ):
        ...
        if function_name not in samples_list:
            ...
            message = (
                '[ERROR] An unexpected situation occurred, '
    ...
    def start_perf_timer(self, function_name: str):
        ...
        self._elapsed_time_samples[function_name].append(elapsed_time_sample)

    def stop_perf_timer(self, function_name: str):
        self._validate_stop_perf_timer_call(
            function_name, self._elapsed_time_samples
        )
        self._elapsed_time_samples[function_name][
            -1
        ].end_time = time.perf_counter_ns()
        self._update_log_file(
            function_name, self._elapsed_time_samples[function_name][-1]
        )
...
class _PerfMonitor(ABC):
    ...
    def _update_log_file(
        self, timestamp, compute_utilization, memory_utilization
    ):
        with open(
            self.csv_log_output_file_path, 'a', encoding='utf-8'
        ) as o_file:
            o_file.write(
                (f'{timestamp}, {compute_utilization}, {memory_utilization}\n')
            )
            self._number_log_file_writes += 1
    ...
    def _update_compute_utilization_samples(
        ...
        with self._metrics_data_lock:
            ...
            if (
                len(self._compute_utilization_samples)
    ...
    def _update_allocated_memory_bytes_samples(
        self, timestamp: int, value: float
    ):
        if value is not None:
            value = round(value, self._VALUE_PRECISION_DECIMAL_LIMIT)
        with self._metrics_data_lock:
            ...
            if (
                len(self._allocated_memory_bytes_samples)
                > self._MAX_NUMBER_RECORDS_IN_MEMORY
            ):
    ...
    def start_metrics_polling(self, nop_time_seconds: float = None):
        ...
        def _metrics_polling():
            ...
            while self._poll_metrics:
                (timestamp, compute_utilization, allocated_memory_bytes) = (
                    self._collect_metrics()
                )

                self._update_compute_utilization_samples(
                    timestamp, compute_utilization
                )

                self._update_allocated_memory_bytes_samples(
                    timestamp, allocated_memory_bytes
                )

                memory_utilization = None
                if allocated_memory_bytes is not None:
                    memory_utilization = (
                        100.0 * allocated_memory_bytes
                    ) / self._total_memory
                    memory_utilization = round(
                        memory_utilization, self._VALUE_PRECISION_DECIMAL_LIMIT
                    )

                compute_utilization = round(
                    compute_utilization, self._VALUE_PRECISION_DECIMAL_LIMIT
                )
                self._update_log_file(
                    timestamp, compute_utilization, memory_utilization
                )
...
class CPUPerfMonitor(_PerfMonitor):
    ...
    def __init__(self, monitor_name):
        super().__init__(
            monitor_name, device_type=PerfMonitorDeviceType.CPU_DEVICE
        )
...
class AppleSiGpuPerfMonitor(_PerfMonitor):
    ...
    def __init__(self, monitor_name, gpu_index: int = 0):
        super().__init__(
            monitor_name, device_type=PerfMonitorDeviceType.GPU_DEVICE
        )
        ...
        if self._has_gpus:
            if (gpu_index < 0) or (gpu_index >= self._number_gpus):
                message = (
                    '[ERROR] Invalid index for GPU selection.\n'
