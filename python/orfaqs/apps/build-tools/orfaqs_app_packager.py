...
class ORFaqsAppsPackager(ORFaqsApp):
    ...
    def _package_name_path_map() -> dict[str, pathlib.Path]:
        ...
        if not _AVAILABLE_PACKAGES_NAME_PATH_MAP:
            orfaqs_app_directory = (
                DirectoryUtils.make_path_object(__file__).parent.parent
            )
    ...
    def cli():
        try:
            ...
            if (((ui_args.get('package_name') is not None) and
                (ui_args.get('build_all') or
                    ui_args.get('available_packages'))) or
                    ((ui_args.get('build_all') and
                      ui_args.get('available_packages')))):
                message = ('[ERROR] Conflicting arguments. '
                           'Pass only one argument to the CLI')
                print(message)
                cli_arg_parser.print_help()
            elif ((ui_args.get('package_name') is None) and
                  (not ui_args.get('build_all')) and
                  (not ui_args.get('available_packages'))):
                cli_arg_parser.print_help()
    ...
    @staticmethod
    def _run(package_name: str = None,
